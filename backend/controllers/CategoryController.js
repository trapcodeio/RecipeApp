...
const CategoryController = {
  ...
  async create(http, boot, error) {
    ...
    if (!category) return error("Name of category is required");
    ...
    if (hasCategory) return error(`Category with name (${category}) already exists.`);
  ...
  async delete(http, boot, error) {
    ...
    if (!categoryId) return error("Category ID is missing! reload browser and try again");

    const category = await Categories.findById(categoryId);
    if (!category)
      return error(
        `No category found with id: (${categoryId}), maybe already be deleted`
      );
    ...
    if (hasRecipes)
      return error(
        `Category (${nameOfCategory}) has recipes in it, delete recipes before deleting this category.`
      );
  ...
  async rename(http, boot, error) {
    ...
    if (!name) return error("New name of category is required");
    ...
    return http.toApi({
      message: `Category (${oldName}) renamed to (${name}) successfully`
    });
  }
};
